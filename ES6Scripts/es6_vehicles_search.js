let vehicles = [
    ...
    {
    color: "Red",
    type: "Pickup Truck", registrationState: "TX", licenseNo: "A1D-2NC",
    ...
    registrationExpires: new Date("5-30-2022"), capacity: 5
    },
    {
    color: "White",
    type: "Pickup Truck", registrationState: "TX", licenseNo: "CAC-7YT",
    registrationExpires: new Date("1-31-2022"), capacity: 5
    },
    {
    color: "White",
    type: "Pickup Truck", registrationState: "CA", licenseNo: "123-ABC",
    registrationExpires: new Date("3-31-2023"), capacity: 5
    }
    ];

    // Which vehicles are RED?
    let redVehicles=vehicles.filter(v=>v.color=="Red");
    console.table(redVehicles);
    console.log("-------------------------------------------");
// Which vehicles have registrations that are expired?

let registrationExpiredVehicles=vehicles.filter(v=>v.registrationExpires<=new Date());
console.table(registrationExpiredVehicles);
console.log("-------------------------------------------")
// Which vehicles that hold at least 6 people?
let vehicleHold6People=vehicles.filter(v=>v.capacity>=6);
console.table(vehicleHold6People);
console.log("-------------------------------------------")

// Which vehicles have license plates that end with "222"?
let vehicleEndWIth222=vehicles.filter(v=>v.licenseNo.includes("222"));
console.table(vehicleEndWIth222)
